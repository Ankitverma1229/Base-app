...
const Login = ({ switchToRegister }) => {
    ...
    return (
        <div className={`w-full flex flex-col md:items-start justify-center px-5  lg:ps-24 ${isDarkMode ? 'text-[#FFFF]' : 'text-[#000000]'}`}>
            ...
            <div className='mt-5'>
                <div className='flex justify-between  md:gap-4 mb-6'>
                    <button 
                        ...
                        onClick={googleLogin} 
                    >
                        <img src={googleLogo} alt="Google Icon" className='w-4 h-4 lg:mr-3' />
                        <span className='text-[#858585] font-montserrat font-medium text-xs lg:text-sm'>Sign in with Google</span>
                    </button>
                    <button 
                        className={`flex items-center gap-1 md:gap-0 justify-center ${isDarkMode ? 'bg-[#0D0D0D]' : 'bg-white'} shadow-md px-3 md:px-4 py-2 rounded-lg ${isDarkMode ? 'hover:bg-neutral-800' : 'hover:bg-gray-100'} transition`}
                    >
                        <img src={appleLogo} alt="Apple Icon" className='w-4 h-4 lg:mr-3' />
                        <span className='text-[#858585] font-montserrat font-medium text-xs lg:text-sm'>Sign in with Apple</span>
                    </button>
                </div>
                <div className={`${isDarkMode ? 'bg-[#0D0D0D]' : 'bg-white'} rounded-xl shadow-lg w-full p-7`}>
                    <form className='flex flex-col gap-5' onSubmit={handleSubmit}>
                        ...
                        <div>
                            <a href="#" className='text-[#346BD4] font-montserrat font-semibold text-sm hover:underline'>Forgot Password?</a>
                        </div>
                        <button 
                            type="submit" 
                            className={`bg-[#605BFF] ${isDarkMode ? 'text-[#000000]' : 'text-[#FFFF]'} font-montserrat font-semibold text-md py-2 rounded-xl w-full hover:bg-[#4e47e5] transition`}
                        >
                            Sign In
                        </button>
                    </form>
                ...
                <p className='text-center text-xl md:text-sm mt-10 font-lato text-[#858585]'>
                    ...
                    <a href="#" onClick={switchToRegister} className='text-[#346BD4] font-montserrat font-semibold  hover:underline'><br className='md:hidden'></br> Register here</a>
                </p>
            </div>
        </div>
    );
